...
export default function Customers(){
    ...
    async function handleDelete(vehicleNumber) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Delete action failed with status ${response.status}`)
            }
    ...
    const displayedVehicles = vehicles
        .filter((vehicle) => vehicle.status !== null && vehicle.status !== undefined && Number(vehicle.status) === 1)
        .sort((a, b) => {
